...
namespace Cradle.Players
{
	...
	{
		private string SpeakerTag = "SpeakerTag";
		public Story Story;
		public RectTransform Container;
		public Button LinkTemplate;
		public Text TagTemplate;
		...
		void Start()
		{
			...
			TagTemplate.rectTransform.SetParent(null);
			TagTemplate.rectTransform.hideFlags = HideFlags.HideInHierarchy;
			...
			if (output is StoryText)
			{
				...
				if (!string.IsNullOrEmpty(text.Text))
				{
					...
					foreach (Match m in rx_splitText.Matches(text.Text))
					{
						...
						uiWord.text = word;
						uiWord.name = word;
						if (first && (word == "Gina" || word == "Jordy" || word == "Player" || word == "Thoughts" || word == "Anxiety" || word == "Emily" || word == "Rea"))
						{
							uiWord.fontSize = uiWord.fontSize + 5;
							first = false;
						}
